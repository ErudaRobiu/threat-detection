...
The planned corpus comprised 1,000 URLs and 600 emails. After the URL classes were normalised to their registrable domains, to remove a collection artefact described in Section 4.7.5, and de-duplicated, the harness drove 1,479 items. The run reached all of them over approximately 8.8 hours, but the free-tier rate limit on the semantic model degraded a share of items to rule-only scores, which the harness deliberately did not cache because a degraded score is not a genuine semantic reading. Because that degradation fell most heavily on the email class, which was processed last, a second pass was run that re-scored the email items first, and this resumed pass raised the number of cleanly scored emails substantially before it too was stopped. The detection results below therefore rest on the 1,151 items that scored cleanly across both passes, whose composition is given in Table 4.5. Email coverage remains partial, at 427 of the 600 email items, and this truncation is a material limitation treated as such here and in Chapter Five.

**Table 4.5: Composition of the Clean Evaluation Set (n = 1,151)**
...
The email class was initially thin, with only 19 phishing emails scoring cleanly before the resumed pass, because the email corpus was processed last when rate limiting was heaviest. The email-first resume raised the phishing-email count to 130, which places the semantic and fusion conditions on an adequate positive class rather than a token one; the material change this produced in the central gate result is reported in Section 4.7.4. A separate hard-case corpus of 100 items, evenly split between legitimate and threat, was constructed to supply the borderline cases that the main corpus is sparse in; approximately 22 per cent of its items are authored rather than sourced, a proportion disclosed here because those items were written for the study.
...
| 2 | AI only (A), email | 423 | 0.823 | 0.960 | 0.886 | 0.088 | 0.979 |
| 3 | HTSA, gamma = 0 | 1151 | 0.867 | 0.662 | 0.751 | 0.071 | 0.801 |
| 4 | HTSA, gamma = 0.2 | 1151 | 0.880 | 0.649 | 0.747 | 0.062 | 0.804 |
| 5 | Random Forest, domain-only | 1000 | 0.743 | 0.654 | 0.696 | 0.226 | 0.767 |
| 6 | HTSA-E | 1151 | see 4.7.4 | | | | |

Several features of Table 4.6 require comment. The rule layer alone achieves high precision, 0.817, but low recall, 0.482, which is the empirical signature of structural detection: when the rule engine fires it is usually right, but it misses a large share of threats, because modern phishing hosted on clean infrastructure and text-only social engineering are structurally innocent. This is not a shortfall of the implementation but the motivation for the semantic layer, and it is discussed further in Section 4.8.

The semantic condition is reported on the email corpus only, because the semantic layer abstains on bare URLs and the 724 URL items therefore carry no semantic score. On the 423 email items with a semantic score, of which 126 are phishing, it achieves a precision of 0.823, a recall of 0.960, and an area under the curve of 0.979, making it the strongest single layer on the email class. This is a credible estimate on an adequate positive class; it should be noted that on the earlier 857-item data, before the email-first resume, the same condition returned an area under the curve of 1.000 on only 15 phishing emails, a figure too small to be reliable, and the larger sample supersedes it.
...
The central experimental question was whether the interaction term, the agreement gate, improves detection, and this section reports the ablation that answers it. The result is mixed, and it is reported as such. Before the figures are read, one structural fact must be established, because it governs their interpretation. The gate operates only on submissions for which both R and A are defined, because for any item on which the semantic layer abstains the hybrid score reduces to R and the interaction term is inert. Every URL item in the corpus abstains, so the gate can act on none of them. The entire effect of the gate therefore lives within the 423 email items on which both scores are defined, of which 126 are phishing, and it does not rest on the 1,151 total. This is stated plainly so that no reader concludes otherwise.

On those 423 dual-layer items, the gate produces a precision-for-recall trade rather than a clean gain, as shown in Table 4.7. Moving the interaction weight from zero to 0.2 raises precision from 0.929 to 0.974 by suppressing six of the nine false positives present under the simple average, but it does so at the cost of six true positives, lowering recall from 0.937 to 0.889. The trade is close to one for one.

**Table 4.7: Gate Ablation on the 423 Dual-Layer Email Items**

| Configuration | Precision | Recall | False positives | True positives | False negatives |
|---|--:|--:|--:|--:|--:|
| gamma = 0 (simple average) | 0.929 | 0.937 | 9 | 118 | 8 |
| gamma = 0.2 (agreement gate) | 0.974 | 0.889 | 3 | 112 | 14 |

Across the full clean corpus, where the inert URL items dilute the effect, the same movement raises overall precision from 0.867 to 0.880, lowers the false-positive rate from 0.071 to 0.062, lowers recall from 0.662 to 0.649, and raises the area under the curve marginally, from 0.801 to 0.804. On this corpus, therefore, the gate lowers the false-positive rate and raises precision at a matching cost in recall, and it does not improve the best F1 obtainable under a full threshold sweep, which is 0.754 with or without the interaction term. It is an instrument for trading recall against precision, not a uniform improvement. It is worth recording that an earlier reading on the 857-item data, before the email-first resume enlarged the phishing-email class from 15 to 126, suggested a clean gain of three false positives suppressed at no cost in true positives; the larger sample resolves that into the precision-for-recall trade reported here, which is the more reliable finding.
...
On an earlier 48-item sample the gate produced no change in classification at all, with identical binary metrics, an identical best F1 of 0.818 under a full threshold sweep, and an equal area under the curve of 0.825, although a mean suppression of 0.044 confirmed that the mechanism was acting. Taken together, the three samples describe a mechanism whose effect is real but modest and corpus-dependent: no measurable effect on the smallest sample, a net-harmful trade on the adversarial borderline set, and a precision-for-recall trade on the main corpus. One finding was consistent across every sample: raising the interaction weight above 0.2 was unambiguously harmful, driving recall down without a compensating gain in precision. The value of 0.2 is therefore an upper bound on the useful weight rather than a point on an improving curve. Figure 4.14 and Figure 4.15 present the condition comparison and the confusion matrices, and Figure 4.16 presents the gamma sweep.
...
A live phishing email encountered during the study exposed a structural limit of the fusion that the gate cannot address, and that motivated an escalation variant, HTSA-E, reported here as the sixth condition. Because the hybrid score is a weighted sum, a structurally clean email, one with R close to zero, is capped at H = beta * A, which at gamma = 0.2 is 0.40, so the score cannot reach High no matter how certain the semantic layer is. Weak safety evidence, a low R that means only that a sender controls their own domain, can thereby veto strong threat evidence, a high A that means the language is unambiguously fraudulent. Across the combined evaluation set, 73 items had R below 0.15 and A above 0.8 and were held at Medium by this cap, and 71 of them were genuine threats. HTSA-E adds a disjunctive escalation on top of the existing gate: if either layer is independently certain, exceeding a high threshold tau, the score is escalated. The threshold was not chosen by hand but derived from the data as the lowest value at which no legitimate training item escalates, which on the enlarged training split is tau = 0.76, set just above the highest legitimate escalation score of 0.75. This threshold fell from the value of 0.910 obtained on the smaller pre-resume data, because a legitimate internal-business message scoring 0.75 lowered the legitimate ceiling once the email class was thickened, which confirms that the earlier threshold had been anchored to an unrepresentative sample. Applying HTSA-E moved 112 threats from Medium to High or Critical and moved 24 further threats up a tier, a substantially larger benefit than the smaller data suggested. It is, however, no longer cost-free: three legitimate items escalated on held-out data, an honest generalisation gap in a threshold derived from a training split. At the 0.3 clearance threshold its binary metrics are identical to the gated hybrid, because it acts at the Medium-to-High boundary, above the clearance line, so its contribution is a tier change rather than a change in what is cleared. It is reported here as the delivered refinement of the fusion, with the caveat that both its threshold and its small legitimate cost would need to be re-established on a fully clean corpus.
...
The interaction term is the point on which the study is most instructive, and its lesson is methodological. The term was proposed in Chapter Three as an amplifier of agreement, and implementation established by formal analysis that it cannot amplify at all, that it is instead an agreement gate which suppresses disagreement. The evaluation of that gate then produced a mixed result: no effect on the smallest sample, net harm on the adversarial hard-case corpus, and, on the main corpus once its phishing-email class had been enlarged, a precision-for-recall trade rather than a uniform gain, with any weight above 0.2 harmful throughout. The character of that main-corpus finding itself changed as the data grew, from an apparent clean gain on 15 phishing emails to a near one-for-one trade on 126, which is a reminder that a result resting on a small positive class should be held provisionally. A rigorous account of testing one's own proposed mechanism and finding its benefit modest and conditional is a stronger contribution than a formula reported to have worked on first presentation, and the mixed result is therefore presented as a finding rather than smoothed toward a positive one. It should also be read in the light of the corpus limitation, since the gate's effect lives in 423 email items of which 126 are phishing, and email coverage is still partial, so a fully clean run is required before its value is settled.
